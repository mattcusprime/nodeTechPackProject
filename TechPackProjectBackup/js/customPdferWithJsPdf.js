...
function docProcessor(garmentProduct) {
    ...
    var header = function () {
        ...
        if (frontImage) {
            ...
            jsPdfDoc.addImage(frontImage, 'PNG', xMarginReset, 0, numFrontBackWidth, 25, 'front', 'fast');
        ...
        jsPdfDoc.text(xMarginReset, yMarginReset, garment.name);
        jsPdfDoc.line(xMarginReset, yMarginReset + 1, numLineLength + numLineLength, yMarginReset + 1);
        yMarginReset += numHeaderYoffset;
        jsPdfDoc.text(xMarginReset, yMarginReset, garment.activeSource);
        jsPdfDoc.line(xMarginReset, yMarginReset + 1, numLineLength + numLineLength, yMarginReset + 1);
        yMarginReset += numHeaderYoffset;
        jsPdfDoc.text(xMarginReset, yMarginReset, garment.activeSpecName);
        jsPdfDoc.line(xMarginReset, yMarginReset + 1, numLineLength + numLineLength, yMarginReset + 1);
        yMarginReset += numHeaderYoffset;
    ...
    this.addFrontBackImages = function () {
        ...
        for (var i = 0; i < arrOfImages.length; i++) {
            ...
            jsPdfDoc.addImage(arrOfImages[i], 'PNG', numNewX, this.yPosition, numImageWidth, numImageHeight, i.toString(), 'fast');
    ...
    if ($("#frontBackImages img").length) {
        frontImage = $('#frontSketch img:first').attr('src');
    }
    else {
        frontImage = false;
    };
    var backImage;
    if ($("#frontBackImages img").length) {
        backImage = $('#frontSketch img:last').attr('src');
    }
    else {
        backImage = false;
    };
    ...
    this.addPageAndReset = function (sectionName) {
        if (!garmentProduct.isCurrentSpecAnActiveSpec && !garmentProduct.isCurrentSpecAnAvailableSpec) {
            //imgBase64DevImage
            jsPdfDoc.addImage(imgBase64DevImage, 'PNG', 0, 0, jsPdfDoc.internal.pageSize.width, jsPdfDoc.internal.pageSize.height, 'fast');
        };
        ...
        if (typeof (sectionName) != 'undefined') {
            ...
            jsPdfDoc.outline.add(this.node, sectionName, { pageNumber: numPage });
        };
        //this.footer();
        this.pageNumber++;
        jsPdfDoc.addPage();
    ...
    this.createPageOne = function () {
        ...
        //jsPdfDoc.setDrawColor(179, 204, 175);
        //jsPdfDoc.setFillColor(this.colors.headerGray);
        //this.drawColorByName('black');
        //this.fillColorByName('headerGray');
        //jsPdfDoc.setLineWidth(1);
        jsPdfDoc.rect(numInitialLeftOfGrid, this.yPosition - numLineHeight, numInitialRightOfGrid, heightOfLines);
        //this.fillColorByName('white');
        //this.drawColorByName('black');
        //jsPdfDoc.setFillColor(this.colors.white);
        //jsPdfDoc.setDrawColor(255, 255, 25);
        ...
        //this.movePositions(xOffsetToCenter,yMarginReset);
        // put to top middle
        //jsPdfDoc.text(this.xPosition, this.yPosition, 'Brand:' + this.pageOneAttributes.Brand);
        this.checkTextLength(this.pageOneAttributes.APS_Corp_Division, 25, 13);
    ...
    this.addRoutingTable = function () {
        if ($("#routing").length) {
            ...
            //pageOneColumns.push(objContentSizeTbl);
            //pageOneColumnsSectionTwo.push(objContentSizeTbl);
            var arrOfColumnWidths = [15, 40, 40, 40, 20, 40, 40,70];
            jsPdfDoc.setFontSize(8);
    ...
    this.addRevisionTable = function () {
        if ($("#revisionAttributeTbl").length) {
            ...
            //this.yPosition += 20;
            jsPdfDoc.setFontSize(6);
            var arrOfColumnWidths = [15, 50, 50, 50, 35, 95, 35, 35];
        ...
        if ($(".construction").length) {
            ...
            for (var i = 0; i < arrOfConstructions.length; i++) {
                try {
                    ...
                    var arrOfColumnWidths = [65, 40, 45, 25, 45, 30, 30, 30, 40, 35];
    ...
    this.addLabelBom = function () {
        if ($("#labelBom").length) {
            ...
            //pageOneColumns.push(objContentSizeTbl);
            //pageOneColumnsSectionTwo.push(objContentSizeTbl);
            var arrOfColumnWidths = [45, 25, 35, 25, 35, 55, 20, 25, 25, 25, 25];
            jsPdfDoc.setFontSize(8);
    ...
    this.processAnImageWithATitleInHeaderValue = function (selector) {
        ...
        if ($(selector).length) {
            ...
            for (var i = 0; i < arrOfImageObjectsWithTitles.length; i++) {
                ...
                this.processTheTitleAndItsImage(objCurrent.decodedHeader, objCurrent.imageSrc, numX, yMarginReset, objCurrent.width, objCurrent.height);
            };
        };
    };
    ...
    this.runAndSave = function () {
        ...
        //var numContentBreakSize = yMarginReset;
        var margin = 0.5;
        //jsPdfDoc.setHeaderFunction(this.header);
        ...
        this.resetXbutSpecifyY(this.yPosition);
        jsPdfDoc.outline.add(this.node, 'General Attributes', { pageNumber: jsPdfDoc.internal.pages.length - 1 });
        /*var dooper = function () {
            this.header();
        };*/
