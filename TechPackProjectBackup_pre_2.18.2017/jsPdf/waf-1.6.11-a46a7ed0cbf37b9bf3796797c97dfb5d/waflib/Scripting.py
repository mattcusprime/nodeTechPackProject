...
def waf_entry_point(current_directory,version,wafdir):
	...
	while cur:
		...
		if not Context.run_dir:
			if Context.WSCRIPT_FILES.intersection(lst):
				Context.run_dir=cur
	...
	if not Context.run_dir:
		if'-h'in sys.argv or'--help'in sys.argv:
			...
			ctx.parse_args()
			sys.exit(0)
		Logs.error('Waf: Run from a directory containing a file named %r'%' or '.join(Context.WSCRIPT_FILES))
		sys.exit(1)
	try:
		...
		sys.exit(1)
	try:
		set_main_module(os.path.join(Context.run_dir,Context.WSCRIPT_FILES.intersection(os.listdir(Context.run_dir)).pop()))
