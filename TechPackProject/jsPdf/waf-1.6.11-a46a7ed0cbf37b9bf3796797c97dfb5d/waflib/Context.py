...
WSCRIPT_FILES=set([WSCRIPT_FILE,WSCRIPT_FILE+'.py'])
launch_dir=''
run_dir=''
top_dir=''
out_dir=''
...
class Context(ctx):
	...
	def _grow_env(self):
		if not self._default_env:
			...
			c=waflib.Configure.ConfigurationContext()
			top=c.path.abspath()
			self._default_env=c.env
			self._default_env[TOP]=top
			self._default_env[TOP.upper()]=top
		return self._default_env
	_default_env=None
	env=property(_grow_env)
	...
	def recurse(self,dirs,name=None,mandatory=True,once=True):
		...
		for d in Utils.to_list(dirs):
			...
			_lsd=os.listdir(d)
			_matches=WSCRIPT_FILES.intersection(_lsd)
			_matches_fn=set([_e+'_'+(name or self.fun)for _e in WSCRIPT_FILES]).intersection(_lsd)
			node=None
			if _matches_fn:
				node=self.root.find_node(os.path.join(d,_matches_fn.pop()))
				...
				node=self.root.find_node(os.path.join(d,_matches.pop()))
				...
				if node and(not once or tup not in cache):
					...
					try:
						...
						self.post_recurse(node)
			elif not node and mandatory:
				raise Errors.WafError('No script file in directory %s'%d)
...
def load_module(path):
	try:
		...
		pass
	module=imp.new_module(WSCRIPT_FILES.intersection(os.listdir('.')).pop().split('.')[0])
	try:
